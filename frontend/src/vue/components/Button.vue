...
<template>
    ...
    <component :is="leadingIcon" v-if="leadingIcon && !processing" class="size-4 stroke-1 shrink-0 rtl:scale-x-[-1]" />
    <LoaderCircle v-if="processing" class="h-4 w-4 animate-spin" />
    <span class="truncate">{{ label }}</span>
    <!-- scale-x-[-1] is for rtl languages to flip the icon -->
    <component :is="trailingIcon" v-if="trailingIcon && !processing" class="size-4 stroke-1 shrink-0 rtl:scale-x-[-1]" />
    </button>
</template>